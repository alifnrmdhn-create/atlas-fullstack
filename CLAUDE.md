...
8. **Anti silent-drop / no-op senyap** — DILARANG endpoint mutasi "sukses diam-diam" saat efek intinya tak terjadi. Dua rupa: **(A)** FE kirim field, validator BE (`$request->validate([...])`) tak punya rule-nya → ter-buang senyap, edit jadi no-op walau 2xx. Tiap edit modal: **rekonsiliasi keys payload FE vs `validate()`** (mismatch `store()` vs `update()` = jebakan klasik — `update()` WAJIB mirror field editable dari `store()`). **(B)** side-effect kondisional (notif/broadcast/state-change) di-skip tapi response tetap 2xx, menyesatkan caller seolah berhasil. Aturan: bila efek inti tak jadi, **KABARKAN** (422/pesan), jangan 2xx kosong. **Test mutasi WAJIB assert efek TERAMATI** (`assertDatabaseHas`/notif/state berubah), bukan cuma status 2xx — kalau cuma cek status, silent-drop lolos. Latar (recurring): KPI edit buang `isLeadingIndicator`, "Give support to PIC" mute tanpa kirim saat owner=self, hardening 2026-06-17 (code/plannedWeeks/apmsKpiCodes — memory `project_program_editability_hardening`). Tak ada gate otomatis (tak regex-detectable) — ini convention + review-check.
