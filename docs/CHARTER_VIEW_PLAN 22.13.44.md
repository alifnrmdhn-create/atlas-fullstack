...
git checkout -b feat/charter-view
```

---

## 3. Setup CLAUDE.md addendum

Sebelum mulai, tambahkan blok ini ke `CLAUDE.md` agar Claude Code di IDE punya konteks:
...
> Baca `docs/CHARTER_VIEW_PLAN.md` section 4 (PHASE 1). Implementasikan migration, model update, config, dan form field sesuai spec. Setelah selesai, run `php artisan migrate` lalu `php artisan test --filter=StrategicFields`. Laporkan hasilnya sebelum lanjut ke Phase 2.
...
> Baca `docs/CHARTER_VIEW_PLAN.md` section 7 (PHASE 4). Jalankan audit grep di 7.2, tampilkan hasilnya. Setelah saya review, baru lakukan replace per 7.3. Jangan replace yang ambigu — flag dulu ke saya.
