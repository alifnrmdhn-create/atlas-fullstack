// One-off visual smoke for Pusat Bantuan (/panduan) — index top, scrolled, topik detail.
...
try {
  ...
  await navigate(page, `${baseUrl}/panduan`)
  await waitFor(page, () => document.querySelector('.panduan__hero-cards') && document.querySelectorAll('.panduan__topik-card').length >= 3, 15000, 'panduan index')
  await sleep(1200)
  await shot(page, join(outDir, 'panduan-top.png'))

  // scroll to FAQ band
  await page.send('Runtime.evaluate', { expression: `document.querySelector('.workspace__content').scrollTo(0, 720)` })
  await sleep(500)
  await shot(page, join(outDir, 'panduan-scrolled.png'))

  await page.send('Runtime.evaluate', { expression: `document.querySelector('.workspace__content').scrollTo(0, 1200)` })
  await sleep(500)
  await shot(page, join(outDir, 'panduan-scrolled2.png'))

  // open a topik detail (first card)
  await page.send('Runtime.evaluate', { expression: `document.querySelector('.workspace__content').scrollTo(0, 0)` })
  await sleep(300)
  await page.send('Runtime.evaluate', { expression: `document.querySelector('.panduan__topik-card').click()` })
  await waitFor(page, () => document.querySelector('.panduan__langkah'), 8000, 'topik detail')
  await sleep(900)
  await shot(page, join(outDir, 'panduan-detail.png'))

  console.log('OK shots: panduan-top.png, panduan-scrolled.png, panduan-scrolled2.png, panduan-detail.png in', outDir)
} catch (e) {
