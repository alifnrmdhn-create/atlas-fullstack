// Visual smoke: badge sidebar = sinyal butuh-aksi seragam (Assignment dapat
// badge, Programs tidak). Login user, dump tiap nav item {label, badge, urgent},
// screenshot sidebar. PKILL chrome antar run.
// Jalankan: APP_URL=http://localhost:9000 SMOKE_LOGIN_ID=lisa.pira.ariska SMOKE_LOGIN_PASSWORD=Password123! node scripts/sidebar-badge-shot.mjs
...
const loginId = process.env.SMOKE_LOGIN_ID ?? 'lisa.pira.ariska'
const loginPassword = process.env.SMOKE_LOGIN_PASSWORD ?? 'Password123!'
const OUT = process.env.OUT_DIR ?? '/tmp/atlas-badge'
...
try {
  ...
  // tunggu overview XHR selesai (badge bergantung myWork/programs/channels)
  await sleep(3500)

  step('dump nav items + badges')
  const items = await evalAwait(page, () => {
    const out = []
    for (const a of document.querySelectorAll('.sidebar__link, a.sidebar__item, .sidebar a')) {
      const labelEl = a.querySelector('.sidebar__label') || a
      const label = (labelEl.innerText || '').trim().split('\n')[0]
      if (!label) continue
      const badgeEl = a.querySelector('.sidebar__badge')
      out.push({
        label,
        href: a.getAttribute('href'),
        badge: badgeEl ? badgeEl.innerText.trim() : null,
        urgent: badgeEl ? badgeEl.classList.contains('sidebar__badge--urgent') : null,
      })
    }
    return out
  })
  ...
  for (const it of items) {
    ...
    seen.add(it.href)
    const tag = it.badge ? `badge=${it.badge}${it.urgent ? ' (urgent)' : ' (neutral)'}` : 'no badge'
    findings.push(`${it.label.padEnd(18)} ${it.href.padEnd(14)} → ${tag}`)
  }

  await capture(page, '01-sidebar', 'Sidebar with badges')
...
// ── CDP helpers (sama dgn notif-dropdown-shot.mjs) ──
