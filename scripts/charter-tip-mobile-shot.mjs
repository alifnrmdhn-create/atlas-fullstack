// Mobile (390px, hover:none, touch) — tap the timeline "now" marker column and
// screenshot the tap-toggled tooltip. Verifies the touch interaction path.
...
const userDataDir = mkdtempSync(join(tmpdir(), 'atlas-chrome-tipm-'))
const chrome = spawn(chromeBin, [
  '--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check',
  '--disable-dev-shm-usage', '--force-device-scale-factor=2',
  '--window-size=420,900', '--remote-debugging-port=0',
...
try {
  ...
  // Emulate a phone: 390px, touch, and crucially hover:none/pointer:coarse so
  // useCanHover() returns false → tap handlers attach.
  await page.send('Emulation.setDeviceMetricsOverride', { width: 390, height: 844, deviceScaleFactor: 2, mobile: true })
  await page.send('Emulation.setTouchEmulationEnabled', { enabled: true, maxTouchPoints: 5 })
  await page.send('Emulation.setEmulatedMedia', { features: [{ name: 'hover', value: 'none' }, { name: 'pointer', value: 'coarse' }] })
  ...
  // Bring the now-marker cell comfortably into view (12-month table overflows
  // on 390px both axes), then dispatch a real bubbling `click` with coords →
  // React onClick → tap-toggle. (Coordinate-based Input.dispatchMouseEvent is
  // unreliable under touch emulation.)
  const pt = await evaluate(page, () => {
    const el = document.querySelector('.atl-cell--mon.atl-col-now, .atl-head--mon.atl-col-now, .atl-col-now')
    if (!el) return null
    ...
    return { x, y, hover: window.matchMedia('(hover: hover)').matches }
  })
  if (!pt) throw new Error('no now-marker cell')
  step(`canHover=${pt.hover} tap (${pt.x},${pt.y})`)
  await new Promise(r => setTimeout(r, 400))

  const shown = await evaluate(page, () => !!document.querySelector('.atl-tip'))
  step(`tooltip visible after tap: ${shown}`)
  ...
  for (const theme of ['light', 'dark']) {
    ...
    writeFileSync(join(outDir, `charter-tip-mobile-${theme}.png`), Buffer.from(shot.data, 'base64'))
    step(`saved charter-tip-mobile-${theme}.png`)
  }
  console.log(`\nDone → ${outDir}`)
} finally {
  chrome.kill('SIGTERM')
...
function step(m) { console.log(`[tipm-shot] ${m}`) }
