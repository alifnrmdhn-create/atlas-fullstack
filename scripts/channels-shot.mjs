// One-off visual smoke for Channels (/channels) — login, open Channels, capture stream + sidebar + composer.
...
const tag = process.env.TAG ?? 'channels'
...
try {
  ...
  await page.send('Runtime.evaluate', { expression: `(() => { const s = document.querySelector('.message-stream'); if (s) s.scrollTo(0, 0) })()` })
  await sleep(600)
  await shot(page, join(outDir, `${tag}-scroll-top.png`))

  console.log(`OK shots: ${tag}-top.png, ${tag}-scroll-top.png in`, outDir)
} catch (e) {
