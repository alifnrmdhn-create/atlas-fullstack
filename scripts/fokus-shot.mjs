// One-off visual smoke for Focus (/fokus) — login as a data-bearing user, capture top + scrolled.
...
const tag = process.env.TAG ?? 'fokus'
...
try {
  ...
  await waitFor(page, () => document.querySelector('.fokus-v2 .fokus-page'), 15000, 'fokus page')
  await sleep(1100)

  await shot(page, join(outDir, `${tag}-top.png`))

  await page.send('Runtime.evaluate', { expression: `document.querySelector('.workspace__content').scrollTo(0, 620)` })
  await sleep(500)
  await shot(page, join(outDir, `${tag}-scroll1.png`))

  await page.send('Runtime.evaluate', { expression: `document.querySelector('.workspace__content').scrollTo(0, 1300)` })
  await sleep(500)
  await shot(page, join(outDir, `${tag}-scroll2.png`))

  console.log(`OK shots: ${tag}-top.png, ${tag}-scroll1.png, ${tag}-scroll2.png in`, outDir)
} catch (e) {
